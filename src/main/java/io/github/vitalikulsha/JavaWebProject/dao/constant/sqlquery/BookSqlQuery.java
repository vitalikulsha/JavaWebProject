...
public class BookSqlQuery extends AbstractSqlQuery {
    ...
    public BookSqlQuery() {
        ...
        String PATTERN = "%%s%";
        this.FIND_BY_TITLE = String.format("SELECT * FROM %s WHERE title LIKE '%%%s%%'",
                Table.BOOK, PATTERN);
        this.FIND_BY_CATEGORY_NAME = String.format("SELECT * FROM %s INNER JOIN %s ON %s.%s=%s.%s" +
                        " WHERE name LIKE '%%%s%%'",
                Table.BOOK, Table.CATEGORY, Table.BOOK, Column.CATEGORY, Table.CATEGORY, Column.CATEGORY_ID,
                PATTERN);
        this.FIND_BY_AUTHOR_NAME = String.format("SELECT * FROM %s INNER JOIN %s ON %s.%s=%s.%s" +
                ...
                Column.FIRSTNAME, PATTERN, Column.LASTNAME, PATTERN);
    }
}
